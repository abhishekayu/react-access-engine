export const metadata = {
  title: 'Quick Start',
  description: 'Get up and running with react-access-control in under 5 minutes.',
};
...
```tsx
import { Can, Feature, AccessGate } from 'react-access-control';
...
```tsx
import { usePermission, useFeature, useRole, usePlan } from 'react-access-control';
