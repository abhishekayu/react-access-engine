...
The `@react-access-control/devtools` package provides a real-time debugging overlay for inspecting access checks, feature evaluations, policy traces, and configuration.
...
```tsx
import { useAccessDebug } from 'react-access-control';
...
```tsx
import { DebugEngine } from 'react-access-control';
